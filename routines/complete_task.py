...
async def go_complete_task(context, action:ActionController, db:DbController):
  dest = await db.get_closest_map(context, content_type="tasks_master", content_code=context.current_hero.task_type)
  if dest and not check_location(context.current_hero, dest.x, dest.y):
    context = await action.hero.move(context, dest.x, dest.y)
      
  context = await action.task.complete_task(context)
