...
async def go_gather(my_hero,controller:ActionController, resource_code, tool_code:str=None):
  ...
  if tool_code and my_hero.weapon_slot != tool_code:
    ...
    my_hero = await go_equip(my_hero, tool_code, controller)

  dest = await controller.get_closest_map(my_hero, content_type="resource", content_code=resource_code)
  ...
  while (check_bag_weight(my_hero.inventory) < max_weight ):
    ...
    my_hero = await controller.gather(my_hero)
